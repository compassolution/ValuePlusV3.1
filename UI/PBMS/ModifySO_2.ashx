<%@ WebHandler Language="C#" Class="ModifySO_2" %>
...
public class ModifySO_2 : HandlerBase, IHttpHandler, System.Web.SessionState.IRequiresSessionState {
            ...
            if (!String.IsNullOrEmpty(strCtrlId))
            {
                ...
                if (CtralIdArrary.Length > 0 && CtralIdArrary.Length == CtralValueArrary.Length)
                {
                    for(int i = 0; i < CtralIdArrary.Length; i++)
                    {
                        ...
                        sbSql.Append("UPDATE SO_2 SET "+strColumnName+" = '"+strColumnValue+"' WHERE SOCODE = '" + strKeyValue + "' and ORDERNUM = '"+strOrderNum+"';");
                        ////从预算中获取成本单价并计算成本总价此逻辑在[USP_Archive_Grid_AfterEdit]中实现
                        //sbSql.Append("exec [USP_PBMS_AfterModify_SO_2] '"+strKeyValue+"','"+strOrderNum+"','edit';");
                    }
                }
    ...
    private void QueryDataList(HttpContext context, String strKeyValue)
    {
        try
        {
            ...
            sbSql.Append(" from SO_2 A WHERE 1=1 ");
            sbSql.Append(" AND SOCODE = '" + strKeyValue + "'");
            sbSql.Append(" order by SOCODE,ORDERNUM ");
            String strSql = sbSql.ToString();
        ...
        try
        {
            ...
            if (!String.IsNullOrEmpty(strSelectedOrderNum))
            {
                ...
                for (int i= 0; i < strSelectArray.Length; i++){
                    ...
                    sbSql.Append("delete from SO_2 WHERE SOCODE = '" + strKeyValue + "' and ORDERNUM = '"+strOrderNum+"';");
                }
            }
            String strSql = sbSql.ToString();
