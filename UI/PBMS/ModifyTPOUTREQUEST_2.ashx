<%@ WebHandler Language="C#" Class="ModifyTPOUTREQUEST_2" %>
...
public class ModifyTPOUTREQUEST_2 : HandlerBase, IHttpHandler, System.Web.SessionState.IRequiresSessionState {
            ...
            if (!String.IsNullOrEmpty(strCtrlId))
            {
                ...
                if (CtralIdArrary.Length > 0 && CtralIdArrary.Length == CtralValueArrary.Length)
                {
                    for(int i = 0; i < CtralIdArrary.Length; i++)
                    {
                        ...
                        sbSql.Append("UPDATE TPOUTREQUEST_2 SET "+strColumnName+" = '"+strColumnValue+"' WHERE TPOUTREQUEST = '" + strKeyValue + "' and ORDERNUM = '"+strOrderNum+"';");
                        ////从预算中获取成本单价并计算成本总价此逻辑在[USP_Archive_Grid_AfterEdit]中实现
                        //sbSql.Append("exec [USP_PBMS_AfterModify_TPOUTREQUEST_2] '"+strKeyValue+"','"+strOrderNum+"','edit';");
                    }
                }
    ...
    private void QueryDataList(HttpContext context, String strKeyValue)
    {
        try
        {
            StringBuilder sbSql = new StringBuilder();
            sbSql.Append("select * ");
            sbSql.Append(" ,(ISNULL(A.HadReceiptQty,0.00) - ISNULL(A.HadRequestQty,0.00)) AS CanRequestQty");
            sbSql.Append(" from TPOUTREQUEST_2 A WHERE 1=1 ");
            sbSql.Append(" AND TPOUTREQUEST = '" + strKeyValue + "'");
            sbSql.Append(" order by TPOUTREQUEST,ORDERNUM ");
            String strSql = sbSql.ToString();
        ...
        try
        {
            ...
            if (!String.IsNullOrEmpty(strSelectedOrderNum))
            {
                ...
                for (int i= 0; i < strSelectArray.Length; i++){
                    ...
                    sbSql.Append("delete from TPOUTREQUEST_2 WHERE TPOUTREQUEST = '" + strKeyValue + "' and ORDERNUM = '"+strOrderNum+"';");
                }
            }
            String strSql = sbSql.ToString();
