//  主调用函数是 vpCalendar_Show(this,[object])和vpCalendar_Show(this)，[object]是控件输出的控件名，举两个例子：
//  一、<input name=txt><input type=button value=vpCalendar_Show onclick="vpCalendar_Show(this,document.all.txt)">
//  二、<input onfocus="vpCalendar_Show(this)">
//  三、vpCalendar_ShowDate(this)//只显示日期
//  四、vpCalendar_ShowDateTime(this)//显示日期时间
...
function vpCalendar_ShowDate(tt) //主调函数1(只显示日期)
{
    bUseTime = false;
    bCanUseTime = false;
    vpCalendar_Show(tt);
...
function vpCalendar_ShowDateTime(tt) //主调函数2（显示日期和时间）
{
    bUseTime = true;
    bCanUseTime = true;
    vpCalendar_Show(tt);
...
function vpCalendar_Show(tt,obj) //主调函数
{
