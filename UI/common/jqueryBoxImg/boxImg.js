...
function InitImageZoom() {
    ...
    var arrPic = new Array(); //定义一个数组【图片路径及名称】
    var arrFileName = new Array(); //定义一个数组【图片名称】
    for (var i = 0; i < len; i++) {
        arrPic[i] = $("img[modal='zoomImg']").eq(i).prop("src"); //将所有img路径存储到数组中
        arrFileName[i] = $("img[modal='zoomImg']").eq(i).attr("fileName"); //将所有img文件名存储到数组中
    }
    ...
    $("img[modal='zoomImg']").each(function () {
        $(this).on("click", function () {
            ...
            $("body").append("<div class=\"mask-layer\">" +
                ...
                "       <div class=\"mask-layer-imgbox-fileName\">标题</div>" +
                "   </div>" +
                "</div>"
            );
            ...
            function showImg() {
                ...
                $(".mask-layer-imgbox-fileName").html(arrFileName[num]); //给弹出框的Img赋值
